...
const urls = [
    ...
    'https://verif.tools/en/poland_passport',
...
async function fetchAllGeneratorInfo() {
  ...
  for (const slug of slugs) {
    try {
      ...
      console.error(`Error fetching info for: ${slug}`, error.message);
      errors.push({ slug, error: error.message });
      // Continue with the next slug instead of stopping the entire process
    }
  }

  // Check if we have fetched any data and write to file
  if (allGeneratorInfo.length > 0) {
    writeDataToFile(allGeneratorInfo, 'generatorInfo.json');
  } else {
    console.log('No generator info to write to file.');
  }

  // Write any errors to a separate file
  if (errors.length > 0) {
    writeDataToFile(errors, 'errors.json');
  } else {
